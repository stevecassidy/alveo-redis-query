...
def get_item_lists():
    ...
    json_text = f.read()
    lists = json.loads(json_text)
    return read_jason(lists)
    
def read_jason(jasonObj):
    result = []
    for lists in jasonObj:
        for itemList in jasonObj[lists]:
...
def get_item_primary_text(url):
    ...
    item = get_item(url)
    if item.has_key('alveo:primary_text_url'):
        if item['alveo:primary_text_url'] != 'No primary text found':
            f = api_request(item['alveo:primary_text_url'])
            return f.read()
        else:
            return None
    else:
...
if __name__=='__main__':
    ...
    for i in get_item_lists():
        ...
        for item in items['items']:
            primaryText = get_item_primary_text(item)
            if primaryText is not None:
                print '\t\t', primaryText[:20].strip()
